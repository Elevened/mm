   ...
   <br/>
        ...
        <div v-if="questionForm.questionType == 1">
          <div class="radio-item" v-for="(option, i) in options" :key="i">
            <el-radio
              :label="option.title"
              :value="option.isRight ? option.title : ''"
            ></el-radio>
          </div>
        </div>
        <div v-if="questionForm.questionType == 2">
          <div v-for="(option, i) in options" :key="i">
            <el-checkbox
              :label="option.title"
              :checked="option.isRight == 1 ? true : false"
            ></el-checkbox>
          </div>
        </div>
        <hr />
      ...
      <span slot="footer">
        <el-button type="primary" @click="handleClose" class="closetBtnLocation"
          >关 闭</el-button
        >
      </span>
    </el-dialog>
...
import {
  list as qlist,
  remove as delquestionById,
  choiceAdd as addById,
  detail as qdetail,
} from "@/api/hmmm/questions"; // 获取基础题库
